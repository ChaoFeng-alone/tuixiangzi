...
void update()
{
	...
	else if (sence == 7 && savecustommap == true)
	{
		savecusmap();
		savecustommap = false;
	}
	else if (sence == 7 && loadcustommap == true)
	{
		loadcusmap(promap);
		loadcustommap = false;
		box_size = 0;
...
void savecusmap()
{
	...
	custom.cusmap_i += 2;
	custom.cusmap_j += 2;
	for (int t = 0; t < custom.cusmap_j-1; t++)
	{
		custommap[0][t] = '=';
		custommap[custom.cusmap_i][t] = '=';
	}
	custommap[0][0] = '|';
	custommap[0][custom.cusmap_j-1] = '|';
	custommap[custom.cusmap_i][0] = '|';
	custommap[custom.cusmap_i][custom.cusmap_j-1] = '|';
	for (int i = 0; i < custom.cusmap_i; i++)
	{
		custommap[i][0] = '|';
		custommap[i][custom.cusmap_j-1] = '|';
	}
	for (int i = 0; i < custom.cusmap_i; i++)
	{
		for (int t = 1; t < custom.cusmap_j-1; t++)
		{
